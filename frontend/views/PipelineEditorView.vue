    ...
    <template v-else-if="model.state.datastreamsError">
      <header class="page-header">
        <div>
          ...
          <p class="page-copy">
            {{ APP_NAME }} authenticated successfully, but it could not load the
            target datastreams needed for mapping.
          </p>
        </div>
      </header>
      ...
      <FeedbackBanner
        :feedback="{ tone: 'error', message: model.state.datastreamsError }"
      />
    </template>
    ...
    <template v-else-if="model.state.datastreams.length === 0">
      <header class="page-header">
        <div>
          ...
          <p class="page-copy">
            Create at least one target datastream in HydroServer first, then
            come back and
    ...
    <template v-else>
      <header class="page-header">
        <div>
          ...
          <p class="page-copy">
            Choose the CSV file you want {{ APP_NAME }} to watch, preview the
            first 50 lines, then click the structure on the right to fill the
            setup form on the left.
          </p>
        </div>
      </header>
        ...
        >
          <div class="pipeline-subcard">
            ...
            <label class="field">
              ...
              />
              <span class="field-hint">
                {{ APP_NAME }} stores the watched file path locally so it can
                keep loading new rows in the background.
              </span>
            </label>
          ...
          <div class="pipeline-subcard">
            ...
            <div class="split-fields">
              <template v-if="model.state.pipelineForm.hasHeaderRow">
                <div :class="model.previewFieldClass('header-row')">
                  ...
                  />
                  <span class="field-hint">
                    Drag the blue HEADER handle in the preview or enter a row
                    number.
                  </span>
                </div>
              </template>
              ...
              <template v-else>
                <div class="field">
                  ...
                  <span class="field-hint">
                    This file is using generated column labels: Column 1, Column
                    2, Column 3...
                  </span>
                </div>
              </template>
              ...
              <div :class="model.previewFieldClass('data-start-row')">
                ...
                />
                <span class="field-hint">
                  Drag the green DATA START handle in the preview or enter a row
                  number.
                </span>
              </div>
            </div>
            ...
            <div :class="model.previewFieldClass('timestamp-column')">
              ...
              />
              <span class="field-hint">
                Drag the amber TIMESTAMP handle in the preview, or click the
                matching header.
              </span>
            </div>
          ...
          <div
            v-if="model.state.pipelineErrors.length > 0"
            class="validation-panel"
          >
            <h3 class="section-title">Fix these issues before saving</h3>
            <ul class="validation-list">
              <li v-for="error in model.state.pipelineErrors" :key="error">
                {{ error }}
              </li>
            </ul>
          </div>
