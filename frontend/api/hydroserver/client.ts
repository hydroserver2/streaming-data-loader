...
export function getBootstrap(): Promise<AppBootstrapResponse> {
  ...
  return Promise.all([getHealth(), getConfig(), getJobs()]).then(
    ([health, config, jobs]) => ({
      health,
      config,
      jobs,
...
export function validateServerUrl(
  url: string
): Promise<ServerUrlValidationResponse> {
  if (isTauriRuntime()) {
    return daemonCommand<ServerUrlValidationResponse>('validate-server-url', {
      url,
    })
  }
...
export function revealFileInFolder(path: string): Promise<ActionResponse> {
  ...
  return Promise.reject(
    new Error(
      'Opening the local file system is only available in the desktop app.'
    )
  )
}
...
export function getDatastreamDetail(
  ...
  return Promise.reject(
    new Error(
      'Expanded datastream metadata is only available in the desktop app.'
    )
  )
}
...
export function enableJob(jobId: string): Promise<ActionResponse> {
  ...
  return requestJson<ActionResponse>(
    `/jobs/${encodeURIComponent(jobId)}/enable`,
    {
      method: 'POST',
    }
  )
...
export function disableJob(jobId: string): Promise<ActionResponse> {
  ...
  return requestJson<ActionResponse>(
    `/jobs/${encodeURIComponent(jobId)}/disable`,
    {
      method: 'POST',
    }
  )
