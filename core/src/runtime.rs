...
impl AppState {
    ...
    pub async fn initialize_async(&self) -> Result<(), String> {
        let _ = self.inner.handle.get_or_init(Handle::current);
    ...
    pub fn start_job_run(&self, job_id: &str, start_message: &str) -> Result<bool, String> {
        ...
        let handle = self
            ...
            .ok_or_else(|| "AppState has not been initialized on a tokio runtime".to_string())?;
